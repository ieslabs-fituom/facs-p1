...
function sidebarColor(e) {
  ...
  for (var t = e.parentElement.children, s = e.getAttribute("data-color"), a = 0; a < t.length; a++) {
    t[a].classList.remove("active");
  }

  e.classList.contains("active") ? e.classList.remove("active") : e.classList.add("active"), document.querySelector(".sidenav").setAttribute("data-color", s), document.querySelector("#sidenavCard") && (e = ["btn", "btn-sm", "w-100", "mb-0", "bg-gradient-" + s], (s = document.querySelector("#sidenavCard+.btn+.btn")).removeAttribute("class"), (_s$classList = s.classList).add.apply(_s$classList, _toConsumableArray(e)));
}

function sidebarType(e) {
  for (var t = e.parentElement.children, s = e.getAttribute("data-class"), a = document.querySelector("body"), n = document.querySelector("body:not(.dark-version)"), a = a.classList.contains("dark-version"), i = [], l = 0; l < t.length; l++) {
  ...
  if (d.classList.add(s), "bg-white" == s) {
    ...
    for (var _e2 = 0; _e2 < v.length; _e2++) {
      v[_e2].classList.add("text-white"), v[_e2].classList.remove("text-dark");
    }
  }
  ...
  if ("bg-default" == s && a) {
    ...
    for (var _e3 = 0; _e3 < v.length; _e3++) {
      v[_e3].classList.add("text-white"), v[_e3].classList.remove("text-dark");
    }
  }
...
function navbarFixed(e) {
  var _s$classList2, _s$classList3;

  var t = ["position-sticky", "bg-white", "left-auto", "top-2", "z-index-sticky"];
  var s = document.getElementById("navbarBlur");
  e.getAttribute("checked") ? (toggleNavLinksColor("transparent"), (_s$classList2 = s.classList).remove.apply(_s$classList2, t), s.setAttribute("data-scroll", "false"), navbarBlurOnScroll("navbarBlur"), e.removeAttribute("checked")) : (toggleNavLinksColor("blur"), (_s$classList3 = s.classList).add.apply(_s$classList3, t), s.setAttribute("data-scroll", "true"), navbarBlurOnScroll("navbarBlur"), e.setAttribute("checked", "true"));
}
...
function toggleNavLinksColor(e) {
  var t = document.querySelectorAll(".navbar-main .nav-link, .navbar-main .breadcrumb-item, .navbar-main .breadcrumb-item a, .navbar-main h6"),
...
function navbarBlurOnScroll(e) {
  ...
  var a = ["bg-white", "left-auto", "position-sticky"],
      n = ["shadow-none"];

  function i() {
    var _t$classList, _t$classList2;

    (_t$classList = t.classList).add.apply(_t$classList, a), (_t$classList2 = t.classList).remove.apply(_t$classList2, n), toggleNavLinksColor("blur");
  }

  function l() {
    var _t$classList3, _t$classList4;

    (_t$classList3 = t.classList).remove.apply(_t$classList3, a), (_t$classList4 = t.classList).add.apply(_t$classList4, n), toggleNavLinksColor("transparent");
  }
...
function darkMode(e) {
  ...
  if (e.getAttribute("checked")) {
    ...
    for (A = 0; A < w.length; A++) {
      w[A].classList.contains("text-white") && w[A].classList.remove("text-white");
    }

    for (A = 0; A < s.length; A++) {
      s[A].classList.contains("light") && (s[A].classList.add("dark"), s[A].classList.remove("light"));
    }

    for (A = 0; A < i.length; A++) {
      i[A].classList.contains("light") && (i[A].classList.add("dark"), i[A].classList.remove("light"));
    }
    ...
    for (A = 0; A < o.length; A++) {
      !o[A].classList.contains("text-white") || o[A].closest(".sidenav") || o[A].closest(".card.bg-gradient-dark") || (o[A].classList.remove("text-white"), o[A].classList.add("text-dark"));
    }
    ...
    for (A = 0; A < v.length; A++) {
      v[A].classList.contains("text-white") && (v[A].classList.remove("text-white"), v[A].classList.remove("opacity-8"), v[A].classList.add("text-dark"));
    }
    ...
    for (A = 0; A < L.length; A++) {
      L[A].classList.remove("border-dark");
    }
    ...
    for (var A = 0; A < s.length; A++) {
      s[A].classList.contains("dark") && (s[A].classList.remove("dark"), s[A].classList.add("light"));
    }

    for (var A = 0; A < i.length; A++) {
      i[A].classList.contains("dark") && (i[A].classList.remove("dark"), i[A].classList.add("light"));
    }
    ...
    for (var A = 0; A < v.length; A++) {
      v[A].classList.contains("text-secondary") && (v[A].classList.remove("text-secondary"), v[A].classList.add("text-white"), v[A].classList.add("opacity-8"));
    }
...
total.forEach(function (i, e) {
  ...
  l.style.padding = "0px", l.style.width = i.querySelector("li:nth-child(1)").offsetWidth + "px", l.style.transform = "translate3d(0px, 0px, 0px)", l.style.transition = ".5s ease", i.onmouseover = function (e) {
    ...
    if (n) {
      var s = Array.from(n.closest("ul").children),
          a = s.indexOf(n) + 1;
